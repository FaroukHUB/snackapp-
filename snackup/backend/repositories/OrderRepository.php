...
class OrderRepository {
    ...
    public static function getActiveOrders(int $restaurantId, int $limit = 50): array {
        $orders = Database::fetchAll(
            "SELECT o.*, c.name as customer_db_name, c.loyalty_code, lr.name as reward_name
             FROM orders o
             LEFT JOIN customers c ON o.customer_id = c.id
             LEFT JOIN loyalty_rewards lr ON o.loyalty_reward_id = lr.id
    ...
    public static function createOrder(int $restaurantId, array $data): int {
        ...
        // Vérifier si une récompense fidélité est demandée (ancien système)
        $loyaltyRewardId = null;
        $legacyLoyaltyPointsUsed = 0;
        ...
        if (!empty($data['loyalty_reward_id']) && $customerId) {
            ...
            if ($reward) {
                ...
                if ($customerPoints >= $reward['points_required']) {
                    ...
                    $legacyLoyaltyPointsUsed = $reward['points_required'];
                }
            }
        }

        // ✅ Nouveau système: points utilisés directement pour payer
        ...
        if (!empty($data['items'])) {
            foreach ($data['items'] as $item) {
                $itemId = Database::insert('order_items', [
                    ...
                    'removed_ingredients' => !empty($item['removed_ingredients']) ? json_encode($item['removed_ingredients']) : null,
                    'selected_drink' => $item['selected_drink'] ?? null,
                    'selected_options' => !empty($item['selected_options']) ? json_encode($item['selected_options']) : null
                ]);
        ...
        if ($customerId) {
            // ✅ Utiliser subtotal pour les stats (montant total de la commande avant réduction points)
            $statsAmount = $data['subtotal'] ?? $data['total'] ?? 0;
            CustomerRepository::incrementStats($customerId, $statsAmount);

            // ⚠️ L'ajout de points fidélité est géré dans orders.php pour éviter les doublons
            // et pour respecter la logique: points gagnés uniquement sur montant payé en DA
            // (100 DA = 1 point, pas de points si payé 100% avec des points)
        }
    ...
    private static function generateOrderNumber(int $restaurantId): string {
        ...
        // ✅ Récupérer le préfixe depuis les settings du restaurant
        $restaurant = RestaurantRepository::getById($restaurantId);
        $prefix = $restaurant['order_prefix'] ?? 'CMD';  // Fallback 'CMD' si non défini
